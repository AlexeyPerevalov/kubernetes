...
func (o *Options) Config() (*schedulerappconfig.Config, error) {
	...
	if checkKubeletConfig(o.Kubelet) {
		//try to set PluginConfig for fit plugin, looks like a hack
		fitPluginConfig := plugins.NewPluginConfig(noderesources.FitName, o.Kubelet)

		//it increase coupling
		for _, profile := range c.ComponentConfig.Profiles {
			profile.PluginConfig = append(profile.PluginConfig, fitPluginConfig)
		}
	}
